// Controller class named 'Country', not 'Countries'. class = description of an instance of..
app.models.Dailies = Ext.regModel('app.models.Dailies', {
  fields: ['name', 'medium', 'size', 'image', 'category'],
  proxy: {
    type: 'fhact',
    reader: 'json',
    id: 'getDalies',
    source: 'localRemote'
  }
});

app.stores.dailies = new Ext.data.Store({
  model: 'app.models.Dailies',
  //autoLoad: true,
  /*
  data: [
         {
           name: '11011 for S&iacute;ol',
           medium: 'Acrylic paint on aluminium',
           size: '89" X 85" 2011',
           image: '392.jpg',
           category: 'aluminiums'
           
         },
         {
           name: '7811',
           medium: 'Acrylic paint on aluminium', 
           size: '99" X 72" Aug 2011',
           image: '391.jpg',
           category: 'aluminiums'
         },
         {
           name: 'RVR',
           medium: 'Acrylic paint on aluminium',
           size: '89" X 49"',
           image: '393.jpg',
           category: 'aluminiums'
         },
         {
           name: 'Accord',
           medium: 'Ground aluminium and acrylic medium',
           size: '19" X 15", 2011',
           image: '394.jpg',
           category: 'aluminiums'
         },
         {
           name: 'Line of Modigliani',
           medium: 'Solid aluminium and acrylic medium',
           size: '20" X 15", 2011',
           image: '385.jpg',
           category: 'aluminiums'

         },
         {
           name: 'Solo',
           medium: 'Aluminium block and acrylic medium',
           size: '19" x 15", 6-6-2011',
           image: '384.jpg',
           category: 'aluminiums'
         },
         {
           name: 'Granite Paint',
           medium: 'Acrylic on aluminium',
           size: '19" x 15", 2011',
           image: '380.jpg',
           category: 'aluminiums'
         },
         {
           name: 'Belfast Painting II',
           medium: 'Acrylic on aluminum',
           size: '13"X10", 2010-2011',
           image: '367.jpg',
           collection: 'Private Collection, New York, USA',
           category: 'aluminiums'
         },
         {
           name: 'For M',
           medium: 'Acrylic on aluminium',
           size: '20" X 16", 2011',
           image: '383.jpg',
           category: 'aluminiums'
         },
         {
           name: 'Ngrange',
           medium: 'Acrylic on solid aluminium',
           size: '06-2009',
           image: '383.jpg',
           category: 'aluminiums'
         },
         {
           name: 'For M',
           medium: 'Acrylic on aluminium',
           size: '20" X 16", 2011',
           image: '383.jpg',
           category: 'aluminiums'
         },
         //Canvases
         {
           name: 'Casts',
           medium: 'Acrylic paint on linen',
           size: '82" X 76", 2011',
           image: '381.jpg',
           category: 'canvases'
         },
         {
           name: 'Monto to Avignon',
           medium: 'Acrylic paint on linen',
           size: '79" X 69", 2010',
           image: '360.jpg',
           category: 'canvases',
           collection: 'Collection of C.MacCreanor'
         },
         {
           name: 'IDIR',
           medium: 'Acrylic on linen',
           size: '77" X 70", 11-2010',
           image: '355.jpg',
           category: 'canvases',
           collection: 'Collection of C.Z. Jones & M. Douglas '
         }
         
  ]*/
});